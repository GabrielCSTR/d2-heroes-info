...
<template>
    <div class="w-full min-h-0 p-3">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="relative h-full p-2 hover:-translate-y-5 transition-all duration-500" v-for="(goodVs, index) in stateProps.goodVsHeroes?.slice(0, 6)"
                :key="goodVs.heroId2">
                <!-- Número do item -->
                <div class="absolute top-0 left-0 bg-green-500 text-white font-bold px-2 py-1 rounded-full">
                    {{ index + 1 }}º
                </div>
